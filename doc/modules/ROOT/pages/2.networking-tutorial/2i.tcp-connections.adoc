...
This is the error you see when connecting to a host that is unreachable, firewalled, or not running a server on the target port. TCP retries the SYN with increasing delays, giving the network every chance to deliver it before the error appears.
...
FIN_WAIT_2:: The other side acknowledged the FIN, but has not yet sent its own FIN. This side is waiting for the remote FIN.
...
. *Reliable termination*: If the final ACK is lost, the remote side retransmits its FIN. The TIME_WAIT state ensures that the local side is still around to re-acknowledge it.
. *Preventing stale segments*: If a new connection is established on the same four-tuple (source address, source port, destination address, destination port) immediately after closing, delayed segments from the old connection might arrive and be misinterpreted as belonging to the new one. TIME_WAIT ensures that enough time passes for any lingering segments to expire.
...
A RST (reset) segment terminates a connection immediately, without a graceful FIN exchange. The receiving side sees an error on the socket -- typically "connection reset by peer."
...
The TCP connection lifecycle is not academic trivia. It is the explanation behind errors you see regularly:
