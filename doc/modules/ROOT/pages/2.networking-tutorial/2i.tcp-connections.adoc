...
This is the error you see when connecting to a host that is unreachable, firewalled, or not running a server on the target port. The long delay before the error appears is TCP being patient -- giving the network every chance to deliver the SYN.
