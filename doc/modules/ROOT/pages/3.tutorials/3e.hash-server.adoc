...
an cpp:io_context[] for network I/O with a `thread_pool` for CPU-bound work,
...
reads and writes is cheap, a single-threaded cpp:io_context[] handles thousands
of connections. But some operations — cryptographic
...
The solution is to keep I/O on the cpp:io_context[] and offload heavy computation
...
* Accepting connections with cpp:tcp_acceptor[]
* Starting independent session coroutines with `run_async`
...
1. **Read** — runs on the cpp:io_context[] thread. The socket awaitable suspends
   the coroutine until data arrives from the kernel.
2. **Hash** — `capy::run( pool.get_executor() )` posts `compute_fnv1a` to the
   thread pool. The coroutine suspends on the cpp:io_context[] and resumes on a
   pool thread. When the task completes, a trampoline posts the
   coroutine back to the cpp:io_context[].
3. **Write** — back on the cpp:io_context[] thread, the hex result is sent to the
   client.
...
2. On `co_await`, the awaitable's `await_suspend` posts the inner task
   ...
   thread.
3. The calling coroutine suspends (the cpp:io_context[] is free to process other
   connections).
...
5. The task's `final_suspend` resumes a trampoline, which calls
   ...
   to the cpp:io_context[].
6. The caller resumes on the cpp:io_context[] thread with the hash result.
...
The accept loop creates a socket per connection and starts a session:
...
cpp:io_context[]. The accept loop immediately continues waiting for the next
connection.
...
The cpp:io_context[] drives all network I/O on the main thread. The thread pool
...
In this example, `run_async` starts the accept loop from `main`, and
