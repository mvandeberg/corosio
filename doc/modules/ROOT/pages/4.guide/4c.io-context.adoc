...
For finer control, construct from an `io_context_options`:
...
To select a specific backend at compile time, construct a
`native_io_context<Backend>` (for example `corosio::native_io_context<corosio::epoll>`)
instead. See xref:4.guide/4c2.configuration.adoc[Configuration] for the full
set of tuning options.
...
=== run_one_for() / run_one_until()

Process at most one work item within a time limit:

[source,cpp]
----
auto n = ioc.run_one_for(100ms);       // One handler, or time out
auto m = ioc.run_one_until(deadline);  // One handler, or reach the deadline
----

These return `0` or `1`. As with `run_one()`, the context must be restarted
with `restart()` before calling them again after they return.
