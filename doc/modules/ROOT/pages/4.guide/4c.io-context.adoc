...
Runtime options control memory recycling behavior. See
xref:4c2.configuration.adoc[Configuration] for all available options.

=== With Explicit Backend

[source,cpp]
----
corosio::io_context ioc(corosio::epoll);    // explicit backend
corosio::io_context ioc(corosio::select);   // portable fallback
----

The template constructor selects a specific I/O backend at compile
time. See xref:4c2.configuration.adoc[Configuration] for compile-time
tuning via custom backend configs.
...
=== Portable Fallback (select)

On platforms without epoll, kqueue, or IOCP, the `io_context` uses
POSIX `select()`:
...
* xref:4c2.configuration.adoc[Configuration] — Compile-time and runtime tuning
