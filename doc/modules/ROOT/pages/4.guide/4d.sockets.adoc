...
The cpp:tcp_socket[] class provides asynchronous TCP networking. It supports
...
the request completes with nothing to do.
...
cpp:tcp_socket[] inherits from cpp:io_stream[], which combines
cpp:io_read_stream[] and cpp:io_write_stream[]. The `read_some` and
`write_some` operations come from those two base classes.
