...
The cpp:tcp_acceptor[] class listens for incoming TCP connections and accepts them
...
the acceptor and socket use the same cpp:io_context[]. Use the 
...
Accept operations support `std::stop_token` through the
xref:4.guide/4b.concurrent-programming.adoc[affine awaitable protocol]:
...
The cpp:tcp_acceptor[] class is the lower-level primitive that cpp:tcp_server[] builds
upon.
