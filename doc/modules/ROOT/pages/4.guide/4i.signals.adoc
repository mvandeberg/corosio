...
Every constructor also accepts an executor instead of an execution context;
...
corosio::signal_set s4(ex);                  // Empty
corosio::signal_set s5(ex, SIGINT, SIGTERM); // With initial signals
----
