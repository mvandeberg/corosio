...
The cpp:signal_set[] class provides asynchronous signal handling. It allows
...
Adding a signal that's already in the set with the same flags has no effect.
...
NOTE: On Windows, only `none` and `dont_care` flags are supported. On POSIX
systems without `SA_NOCLDWAIT`, `no_child_wait` is unavailable. Using
unsupported flags returns `operation_not_supported`.

==== Flag Compatibility

When multiple cpp:signal_set[] objects register for the same signal, they must use
compatible flags:
...
[source,cpp]
----
include::example$snippets/4i_signals.cpp[tag=stop_token,indent=0]
----
...
thread is modifying a cpp:signal_set[].
