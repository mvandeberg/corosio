...
The cpp:tcp_server[] class provides a framework for building TCP servers with
...
cpp:tcp_server[] is a base class designed for inheritance. You derive from it,
...
The cpp:tcp_server::worker_base[worker_base] class is the foundation. It declares a constructor and a
virtual destructor, and holds private intrusive-list bookkeeping used by the server's idle and active
pools. It exposes two pure virtuals you must override:
...
Your worker inherits from cpp:tcp_server::worker_base[worker_base], owns its socket, and implements the
required methods:
...
range of pointer-like objects convertible to a pointer to cpp:tcp_server::worker_base[], such as a
...
Because the range holds a `unique_ptr` to cpp:tcp_server::worker_base[], workers are stored
...
Workers must be provided via `set_workers()` before calling `start()`,
...
cpp:tcp_server[] can listen on multiple ports simultaneously. All ports share
...
The cpp:tcp_server[] class is not thread-safe. All operations on the server
must occur from coroutines running on its cpp:io_context[]. Workers may not be
