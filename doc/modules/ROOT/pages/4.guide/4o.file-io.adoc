...
cpp:stream_file[] for sequential access and cpp:random_access_file[] for
...
cpp:stream_file[] reads and writes sequentially, maintaining an internal
position that advances after each operation. It inherits from cpp:io_stream[],
so it works with any cpp:io_stream[] algorithm.
...
cpp:random_access_file[] reads and writes at explicit byte offsets
...
cpp:stream_file[] additionally provides `seek()` for repositioning.
...
every backend, but what happens next is backend-specific: the POSIX
backends issue `preadv`/`pwritev` and fail at the first read or write
with `ESPIPE`, while io_uring submits `READV`/`WRITEV` at offset `-1`
for a `stream_file` and reads the tty successfully. Adopt one into a
...
* cpp:random_access_file[] supports multiple concurrent reads and writes
  from coroutines sharing the same file object. Each operation is
  independently heap-allocated.
* cpp:stream_file[] allows at most one asynchronous operation in flight at
