...
UDP is a connectionless, message-oriented transport. Each cpp:udp_socket::send[send] produces
exactly one datagram, and each cpp:udp_socket::recv[recv] consumes exactly one datagram. There
...
Simple, Unreliable]. This page covers the Corosio API: cpp:udp_socket[], the
cpp:udp[] protocol tag, and the operations they support.
...
the kernel assigns an ephemeral port on the first cpp:udp_socket::send_to[send_to].
...
cpp:udp_socket::recv_from[recv_from] writes the datagram into the buffer and stores the sender's
...
kernel. Datagrams from any other source are filtered out, and cpp:udp_socket::send[send]/cpp:udp_socket::recv[recv]
...
* **ICMP error reporting** — when a peer is unreachable, the kernel surfaces the resulting ICMP error rather
  than silently discarding it. It appears on a subsequent cpp:udp_socket::send[send] or
  cpp:udp_socket::recv[recv].

== Message Flags

cpp:udp_socket::send_to[send_to], cpp:udp_socket::recv_from[recv_from], cpp:udp_socket::send[send], and cpp:udp_socket::recv[recv] accept an optional
...
| `peek`
| Return data without removing it from the receive queue. The next cpp:udp_socket::recv[recv]
...
| cpp:socket_option::broadcast[broadcast]
| Required before sending to a broadcast address such as
  `255.255.255.255`. Off by default.

| cpp:socket_option::receive_buffer_size[receive_buffer_size] / cpp:socket_option::send_buffer_size[send_buffer_size]
...
| cpp:socket_option::v6_only[v6_only]
...
| cpp:socket_option::join_group_v4[join_group_v4] / cpp:socket_option::leave_group_v4[leave_group_v4]
| Subscribe to or unsubscribe from an IPv4 multicast group.

| cpp:socket_option::join_group_v6[join_group_v6] / cpp:socket_option::leave_group_v6[leave_group_v6]
| IPv6 equivalents.

| cpp:socket_option::multicast_loop_v4[multicast_loop_v4] / cpp:socket_option::multicast_loop_v6[multicast_loop_v6]
| Enable or disable receiving your own outgoing multicast.

| cpp:socket_option::multicast_hops_v4[multicast_hops_v4] / cpp:socket_option::multicast_hops_v6[multicast_hops_v6]
| Set the multicast TTL (IPv4) / hop limit (IPv6). Default is `1` —
  datagrams stay on the local subnet unless you raise it.

| cpp:socket_option::multicast_interface_v4[multicast_interface_v4] / cpp:socket_option::multicast_interface_v6[multicast_interface_v6]
...
A cpp:udp_socket[] permits one outstanding send and one outstanding receive at
a time. Two simultaneous cpp:udp_socket::recv_from[recv_from] calls on the same socket are not
supported. This is enough for the common pattern of a single coroutine that
alternates send and receive. It also suits a pair of coroutines where one
drives outbound traffic and the other drains the receive queue.
...
[cols="1,1,1"]
|===
| Aspect | cpp:tcp_socket[] | cpp:udp_socket[]
...
| Message boundaries
| Not preserved
| Preserved (one cpp:udp_socket::recv[recv] = one datagram)
