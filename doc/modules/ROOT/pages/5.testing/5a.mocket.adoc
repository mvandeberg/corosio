...
A mocket also forwards a few socket-like queries to the underlying socket:

* `is_open()` — whether the underlying socket is open.
* `cancel()` — cancel pending I/O on the underlying socket; outstanding
  operations complete with `capy::cond::canceled`.
* `context()` — the `capy::execution_context` that owns the mocket.
...
* No simulation of network delay; for that, run code under a stop_token
  with a timer.
