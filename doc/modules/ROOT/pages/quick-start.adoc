...
Every Corosio program starts with an cpp:io_context[]. This is the event loop that
...
The cpp:tcp_server[] base class provides connection pooling and lifecycle management.
...
* Workers derive from cpp:tcp_server::worker_base[worker_base] and implement `socket()` and `run()`
...
* xref:3.tutorials/3a.echo-server.adoc[Echo Server Tutorial] — Build the same server with worker pools and design rationale
