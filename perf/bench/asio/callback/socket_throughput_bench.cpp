    ...
    std::size_t chunk_size, double duration_s )
{
    ...
    for( int i = 0; i < num_connections; ++i )
    {
        ...
        write_ops.push_back( std::make_unique<mt_write_op>(
            mt_write_op{ sock1s[i], bufs[i].wbuf1, chunk_size, running } ) );
        read_ops.push_back( std::make_unique<mt_read_op>(
    ...
    for( auto& w : write_ops ) w->start();
    for( auto& r : read_ops ) r->start();
    ...
    if( run_all || std::strcmp( filter, "multithread" ) == 0 )
    {
        ...
        for( auto tc : thread_counts )
        {
            std::string hdr = "Multithread Throughput " +
                std::to_string( tc ) + " threads (Asio Callbacks)";
