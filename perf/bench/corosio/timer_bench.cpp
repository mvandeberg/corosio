...
bench::benchmark_result bench_schedule_cancel(
...
bench::benchmark_result bench_fire_rate(
    perf::context_factory factory, double duration_s )
{
...
bench::benchmark_result bench_concurrent_timers(
    perf::context_factory factory, int num_timers, double duration_s )
{
    ...
    auto timer_task = [&]( int idx, std::chrono::microseconds interval ) -> capy::task<>
    {
        corosio::timer t( *ioc );
        while( running.load( std::memory_order_relaxed ) )
        {
    ...
    double duration_s )
{
    ...
    if( run_all || std::strcmp( filter, "schedule_cancel" ) == 0 )
        collector.add( bench_schedule_cancel( factory, duration_s ) );

    if( run_all || std::strcmp( filter, "fire_rate" ) == 0 )
        collector.add( bench_fire_rate( factory, duration_s ) );

    if( run_all || std::strcmp( filter, "concurrent" ) == 0 )
    {
        perf::print_header( "Concurrent Timers (Corosio)" );
        collector.add( bench_concurrent_timers( factory, 10, duration_s ) );
        collector.add( bench_concurrent_timers( factory, 100, duration_s ) );
        collector.add( bench_concurrent_timers( factory, 1000, duration_s ) );
    }
}
