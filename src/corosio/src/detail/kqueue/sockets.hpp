...
/*
    ...
    -----------------------
    We use persistent kqueue registration: each fd is registered once with
    EVFILT_READ and EVFILT_WRITE and stays registered until closed. Events
    are dispatched to pending operations via descriptor_data.

    Cancellation
    ------------
    See op.hpp for the completion/cancellation race handling via atomics.
    cancel() must complete pending operations (post them with cancelled flag)
    so coroutines waiting on them can resume. close_socket() calls cancel()
    first to ensure this.
...
struct read_initiator
{
    struct promise_type
    {
        kqueue_socket_impl* impl;

        /// Reuse cached frame to avoid per-operation heap allocation.
        static void* operator new(std::size_t n, void*& cached, kqueue_socket_impl*)
        {
            if (!cached)
...
read_initiator make_read_initiator(void*& cached, kqueue_socket_impl* impl);
write_initiator make_write_initiator(void*& cached, kqueue_socket_impl* impl);
...
private:
    /// Check if the socket is at EOF/shutdown condition.
    bool check_eof_condition() const noexcept;
...
using kqueue_sockets = kqueue_socket_service;
