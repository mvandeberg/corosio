...
encrypted_io(corosio::tls_stream& secure)
{
    ...
    char buf[1024];
    auto [ec, n] =
        co_await secure.read_some(capy::mutable_buffer(buf, sizeof(buf)));

    // Write encrypted data
    std::string msg = "Hello, TLS!";
    auto [wec, wn] =
        co_await secure.write_some(capy::const_buffer(msg.data(), msg.size()));
...
capy::task<void>
send_request(corosio::io_stream& stream)
{
...
capy::task<void>
send_request(corosio::tls_stream& stream)
{
    std::string request = "GET / HTTP/1.1\r\n\r\n";
    if (auto [ec, n] = co_await capy::write(
            stream, capy::const_buffer(request.data(), request.size()));
...
capy::task<void>
https_get(
    corosio::io_context& ioc, std::string_view hostname, std::uint16_t port)
{
    // Resolve hostname
    corosio::resolver resolver(ioc);
    auto [resolve_ec, results] =
        co_await resolver.resolve(hostname, std::to_string(port));
    if (resolve_ec)
    ...
    if (auto [ec, n] = co_await capy::write(
            secure, capy::const_buffer(request.data(), request.size()));
        ec)
        throw std::system_error(ec);
